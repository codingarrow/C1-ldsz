...
class Leads_Model extends CI_Model {
	...
	function get_leads_info($lid)
	{		
		$this->db->where('id', $lid);
		return $this->db->get('tbl_leads')->row();
	}
	...
	function get_all_export()
	{
        return $this->db->order_by('last_name, first_name')->get('tbl_leads');
	}
	...
	function get_droplanguage()
    {
		...
		if( $result->num_rows() > 0 )
		{
			foreach($result->result_array() as $row)
			{
				$dropdown[$row['language']]=$row['language'];
			}
		}
		return $dropdown;
	}	  
	...
	function get_droptimezone()
    {
		$result=$this->db->get('timezones');
		$dropdown=array();
		if( $result->num_rows() > 0 )
		{
			foreach($result->result_array() as $row)
			{
				$dropdown[$row['timezone_value']]=$row['timezone_value'];
			}
		}
		return $dropdown;
	}	
	...
	function getallsearch($fname, $lname, $email, $phone)
	{				
		$sql = "SELECT * FROM tbl_leads WHERE first_name like '%$fname%' AND last_name like '%$lname%' AND email like '%$email%' AND phone1 like '%$phone%'";
		$query = $this->db->query($sql);
	...
	{
		$this->db->insert('tbl_leads',$datauser);
